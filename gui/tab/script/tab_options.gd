...
func _ready() -> void:
	...
	Dialogic.Inputs.auto_advance.delay_modifier = Dialogic.Save.get_global_info("auto_advance_modifier", auto)
...
func _open() -> void:
	...
	%Setting_TextSpeed.value = sqrt(text_speed)
	%Setting_AutoSpeed.value = auto
	%Setting_MusicVolume.value = default_music_volume
	%Setting_SoundsVolume.value = default_music_volume
	%Setting_VoiceActing.value = default_voice_acting_volume
...
func _on_setting_text_speed_value_changed(value: float) -> void:
	...
	Dialogic.Save.get_global_info("text_speed", float(text_speed))
...
func _on_setting_auto_speed_value_changed(value: float) -> void:
	...
	auto = float(value)
	Dialogic.Save.set_global_info("auto_advance_modifier", float(auto))
	

## Music 
func _on_setting_music_volume_value_changed(value: float) -> void:
	AudioServer.set_bus_volume_db(AudioServer.get_bus_index("Music"), linear_to_db(value))
	default_music_volume = float(value)
	Dialogic.Save.set_global_info("music_volume", float(default_music_volume))
...
func _on_setting_sounds_volume_value_changed(value: float) -> void:
	...
	default_sound_effects_volume = float(value)
	Dialogic.Save.set_global_info("sound_effects_volume", float(default_sound_effects_volume))
...
func _on_setting_ui_volume_value_changed(value: float) -> void:
	...
	default_voice_acting_volume = float(value)
	Dialogic.Save.set_global_info("voice_acting_volume", default_voice_acting_volume)
