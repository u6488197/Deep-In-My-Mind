...
enum AudioBus { Master, Music, Voice, SFX }
...
func set_effects_bus_volume_linear(volume: float) -> void:
	_set_volume_db(AudioBus.SFX, linear_to_db(volume))

func get_effects_bus_volume_db() -> float:
	return _get_volume_db(AudioBus.SFX)

func get_effects_bus_volume_linear() -> float:
	return _get_volume_linear(AudioBus.SFX)
