...
@export var _send_button : TextureButton
...
func _on_SendButton_pressed():
	...
	if _webhook.start_message() == OK:
		if _message_text.text == "":
			%State.text = "Please input the message"
		else: 
...
func send_report(attach_log_file:=false, attach_analytics_file:=false):
	...
	_webhook.set_embed_title("%s by %s" % ["Bug Reporter", player_id])
	_webhook.set_embed_color(_cfg.get_value("webhook", "color", 15258703))
	
	# add Title
	var title := _mail_line_edit.text.replace("```", "")
	if !title.is_empty():
		_webhook.add_embed_field("Title:", title)
